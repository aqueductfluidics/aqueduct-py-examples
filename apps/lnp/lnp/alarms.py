...
class Alarm:
    ...
    active: bool = False
    _data: "lnp.data.Data" = None  # pointer to Data object
    _devices: "lnp.devices.Devices" = None  # pointer to Devices object
...
class OverPressureAlarm(Alarm):
    ...
    max_pressure_psi: float = 35.0

    cached_aq_pump_rate_ml_min: float = 0.0
    cached_oil_pump_rate_ml_min: float = 0.0
    cached_dilution_pump_rate_ml_min: float = 0.0
    ...
    def condition(self) -> bool:
        ...
        if (
            self._data.P1 > self.max_pressure_psi
            or self._data.P2 > self.max_pressure_psi
            or self._data.P3 > self.max_pressure_psi
        ):
            return True
        else:
            return False
    ...
    def handle(self):
        ...
        print(
            "[***ALARM***] Overpressure alarm raised! Stopping all pumps. Dismiss prompt to continue."
        )
        self.cached_aq_pump_rate_ml_min = self._data.R1
        if (
            isinstance(self._devices.OIL_PUMP, aqueduct.devices.mfpp.obj.MFPP)
    ...
    def restart(self):
        ...
        lnp.methods.pumps_2_and_3_ramp(
            interval_s=1,
            ...
            dilution_pump_start_flowrate_ml_min=self.cached_dilution_pump_rate_ml_min
            / 2,
...
class VacuumConditionAlarm(Alarm):
    ...
    def condition(self) -> bool:
        ...
        if (
            self._data.P1 < self.vacuum_pressure_psi
            or self._data.P2 < self.vacuum_pressure_psi
            or self._data.P3 < self.vacuum_pressure_psi
        ):
            return True
        else:
            return False
    ...
    def handle(self):
        ...
        print(
            "[***ALARM***] Vacuum Condition Alarm raised! Stopping all pumps. Dismiss prompt to continue."
        )
        self.cached_aq_pump_rate_ml_min = self._data.R1
        if (
            isinstance(self._devices.OIL_PUMP, aqueduct.devices.mfpp.obj.MFPP)
...
class BufferVesselEmptyAlarm(Alarm):
    ...
    min_buffer_liquid_mass_g: float = 5.0
    ...
    def condition(self) -> bool:
        ...
        if (
            isinstance(self._data.W2, float)
            and self._data.W2 < self.min_buffer_liquid_mass_g
        ):
            return True
        else:
            return False
    ...
    def handle(self):
        ...
        if (
            ...
            print(
                """[***ALARM***] Low Buffer Vessel Mass alarm raised!
...
class RetentateVesselLowAlarm(Alarm):
    ...
    def condition(self) -> bool:
        ...
        if (
            isinstance(self._data.W2, float)
            and 2 < self.end_expected_retentate_mass_g - self.threshold_g
        ):
            return True
        else:
            return False
    ...
    def handle(self):
        ...
        if (
            ...
            print(
                """[***ALARM***] Retentate Vessel Mass alarm raised!
...
class VolumeAccumulationAlarm(Alarm):
    ...
    def handle(self):
        ...
        if isinstance(rates, lnp.data.TrailingRates):
            rates.print()
    ...
    def set_scale1_target_mass(self) -> None:
        ...
        while n < 5:
            if self._data.W1 is not None:
                print(
                    "[CONTROL] Setting SCALE1 setpoint volume to: {} mL".format(
                        self._data.W1
                    )
                )
    ...
    def check_max_deviation(self, rates: "TrailingRates") -> None:
        if (
            ...
            > self.oil_pump_max_deviation_ml_min
        ):
            print(
                """[CONTROL] Deviation between OIL_PUMP rate: {} mL/min,
    ...
    def handle_mode1(self, rates: "TrailingRates"):
        ...
        if sign is not None:
            ...
            max_adjustment_ml_min = max(
                self._data.R2 * self.max_oil_pump_adjustment_pct,
            ...
            if oil_pump_new_rate is not None and oil_pump_new_rate > 0:
                print(
                    "[CONTROL (MODE {})] changing OIL_PUMP rate to {} mL/min".format(
                        self.mode, lnp.helpers.format_float(oil_pump_new_rate, 2)
                    )
                )
    ...
    def handle_mode2(self, rates: "TrailingRates"):
        ...
        if sign is not None:
            ...
            oil_pump_new_rate = (
                (sign * oil_pump_rate_magnitude_ml_min)
                + self._data.R2
                - rates.W1_ml_min
            )
            max_adjustment_ml_min = max(
            ...
            if oil_pump_new_rate is not None and oil_pump_new_rate > 0:
                print(
                    "[CONTROL (MODE {})] changing OIL_PUMP rate to {} mL/min to "
                    "hit target SCALE1 setpoint of {} mL".format(
                        self.mode,
                        lnp.helpers.format_float(oil_pump_new_rate, 2),
