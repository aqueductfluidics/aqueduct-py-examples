...
def multi_pump_ramp(
    ...
    rate_changes: List[List[float]] = [
        len(pumps) * [0.] for _ in range((number_rate_changes - 1))]

    pump_index = 0
    for (pump, start_rate, end_rate) in zip(pumps, start_flow_rates_ml_min, end_flow_rates_ml_min):
        pump: aqueduct.devices.mfpp.obj.MFPP
        print(
            "[MULTI RAMP] Starting {} at {:.2f} mL/min".format(pump.name, start_rate))
        pump.start(
            mode="continuous",
            direction="forward",
        ...
        rates = local.lib.lnp.helpers.get_flowrate_range(
    ...
    for r in rate_changes:
        ...
        for (pump, rate) in zip(pumps, r):
            pump: aqueduct.devices.mfpp.obj.MFPP
            print(
                "[MULTI RAMP] Adjusting {} to {:.2f} mL/min".format(pump.name, rate))
            pump.change_speed(
                rate_value=rate,
                rate_units="ml_min",
            )
