...
class CoDispenseStation:
    ...
    pump0_output_tubing_volume_ul: float = 350.0
    pump0_priming_volume_ul: float = 151.0 + 300.0 + 200.0

    pump1_input_port: int = 1
    pump1_output_port: int = 2
    pump1_waste_port: int = 3

    pump1_output_tubing_volume_ul: float = 350.0
    pump1_priming_volume_ul: float = 151.0 + 300.0 + 200

    # pump 0 process params
    pump0_volume_to_dispense_ul: float = 36500
    pump0_dispense_rate_ul_min: float = 50.0

    # volume and rate used to quickly prime to the end of the output
    # tubing line for the first dispense
    pump0_output_tubing_prime_volume_ul: float = 315.0
    pump0_output_tubing_prime_rate_ul_min: float = 2000.0

    # rate to use when withdrawing pump0
    pump0_withdraw_rate_ul_min: float = 25000.0

    _pump0_dispensed_ul_counter: float = 0.0
    _pump0_infusions_counter: int = 0
    _pump0_dispense_volume_ul: float = 0

    _realtime_pump0_dispensed_ul_counter: float = 0.0

    # pump 1 process params
    pump1_volume_to_dispense_ul: float = 11600
    pump1_dispense_rate_ul_min: float = 15

    pump1_output_tubing_prime_volume_ul: float = 315.0
    pump1_output_tubing_prime_rate_ul_min: float = 2000.0

    # rate to use when withdrawing pump1
    pump1_withdraw_rate_ul_min: float = 25000.0

    _pump1_dispensed_ul_counter: float = 0.0
    _pump1_infusions_counter: int = 0
    _pump1_dispense_volume_ul: float = 0

    _last_pump0_withdraw_volume_ul: float = 0.0
    _last_pump1_withdraw_volume_ul: float = 0.0

    _pump0_dispense_complete: bool = False
    _pump1_dispense_complete: bool = False

    _realtime_pump1_dispensed_ul_counter: float = 0.0
    ...
    def __str__(self):
        return (
            f"Station {self.index} (mon. {self.pump0_input}, init. {self.pump1_input}): "
    ...
    def make_setpoints(self) -> None:
        ...
        self.pump0_dispensed_recordable = self._aqueduct.recordable(
            name=f"station_{self.index}_pump0_disp.", value=0.0, dtype=float.__name__
        )

        self.pump1_dispensed_recordable = self._aqueduct.recordable(
            name=f"station_{self.index}_pump1_disp.", value=0.0, dtype=float.__name__
        )
    ...
    def record(self, plunger_positions: Tuple[Tuple[int, any, any]]) -> None:
        ...
        #         current_position_ul = (
        #             local.lib.dispensing.methods.get_current_position_ul(
        #                 self._devices.PUMP,
    ...
    def reset_phase_1(self) -> None:
        ...
        """
        self._pump0_dispensed_ul_counter: float = 0.0
        self._pump1_dispensed_ul_counter: float = 0.0
        ...
        if self.phase_setpoint.value == Phase.PHASE_1_INITIALIZED.value:
            ...
            def to_do():
                ...
                local.lib.dispensing.methods.set_valves_and_infuse(
                    ...
                    infuse_rates_ul_min=[50000, 50000],
                    infuse_volumes_ul=[
                        20000,
                        20000,
            ...
            def to_do():
                ...
                local.lib.dispensing.methods.set_valves_and_withdraw(
                    ...
                    withdraw_rates_ul_min=[
                        self.pump0_withdraw_rate_ul_min,
                        self.pump1_withdraw_rate_ul_min,
                    ],
                    withdraw_volumes_ul=[
                        self.pump0_priming_volume_ul,
                        self.pump1_priming_volume_ul,
                    ],
                )

                log_string = (
                    f"Station {self.index}: Priming wdrw {self.pump0_priming_volume_ul} uL pump0 at "
            ...
            self._phase_helper(
                do_if_not_started=to_do, next_phase=Phase.PHASE_1_FINAL_WITHDRAW
            )
            ...
            def to_do():
                ...
                if not pump0_is_active:
                    ...
                    pump0_to_withdraw_vol_ul = (
                        self.pump0_volume_to_dispense_ul
                        - self._pump0_dispensed_ul_counter
                    )
                ...
                if not pump1_is_active:
                    pump1_withdraw_vol_ul = (
                        self.pump1_volume_to_dispense_ul
                        - self._pump1_dispensed_ul_counter
                    )
                    ...
                    self._last_pump1_withdraw_volume_ul = min(
                        pump1_withdraw_vol_ul,
                        local.lib.dispensing.methods.get_syringe_volume_ul(
                            self._devices.PUMP, index=self.pump1_input
                        ),
                    )
                ...
                log_string = (
                    f"Station {self.index}: wdrw {self._last_pump0_withdraw_volume_ul} pump0 at "
                    f"{self.pump0_withdraw_rate_ul_min} "
            ...
            # output tubing prime
            if (
                self._pump0_infusions_counter == 0
                or self._pump1_dispensed_ul_counter == 0
            ):
                self._phase_helper(
                    do_if_not_started=to_do, next_phase=Phase.PHASE_1_INFUSING
                )
            # otherwise, resume dispensing
            else:
                self._phase_helper(
                    do_if_not_started=None, next_phase=Phase.PHASE_1_INFUSING
                )
                repeat = True
            ...
            def to_do(
                pump0_to_dispense_ul: float = None, pump1_to_dispense_ul: float = None
            ):
                """
                ...
                # with N0 mode
                if (
                    self.pump0_dispense_rate_ul_min
                    <= 8
                    * local.lib.dispensing.methods.get_min_rate_ul_min(
                ...
                if (
                    self.pump1_dispense_rate_ul_min
                    <= 8
                    * local.lib.dispensing.methods.get_min_rate_ul_min(
                ...
                if pump0_to_dispense_ul is None:
                    if self._last_pump0_withdraw_volume_ul != 0:
                        self._pump0_dispense_volume_ul = (
                            self._last_pump0_withdraw_volume_ul
                        )
                    else:
                        self._pump0_dispense_volume_ul = (
                            local.lib.dispensing.methods.get_syringe_volume_ul(
                ...
                if pump1_to_dispense_ul is None:
                    if self._last_pump1_withdraw_volume_ul != 0:
                        self._pump1_dispense_volume_ul = (
                            self._last_pump1_withdraw_volume_ul
                        )
                    else:
                        self._pump1_dispense_volume_ul = (
                            local.lib.dispensing.methods.get_syringe_volume_ul(
                ...
                log_string = (
                    f"Station {self.index}: inf {self._pump0_dispensed_ul_counter} "
            ...
            if self.current_phase_status == CurrentPhaseStatus.NOT_STARTED.value:
                ...
                # reload
                    ...
                    self._phase_helper(
                        do_if_not_started=None, next_phase=Phase.PHASE_1_FINAL_WITHDRAW
                    )
            ...
            def to_do():
                ...
                pump0_withdraw_rate_ul_min = min(
                    50000.0,
                    local.lib.dispensing.methods.get_max_rate_ul_min(
                        pump=self._devices.PUMP, index=self.pump0_input
                    ),
                )
                pump0_withdraw_volume_ul = 2 * (
                    self.pump0_output_tubing_volume_ul
                    + self.pump0_output_tubing_volume_ul
                )
                ...
                pump1_withdraw_volume_ul = 2 * (
                    self.pump1_output_tubing_volume_ul
                    + self.pump1_output_tubing_volume_ul
                )
                ...
                local.lib.dispensing.methods.set_valves_and_withdraw(
                    ...
                    ports=[self.pump0_input_port, self.pump1_input_port],
                    withdraw_rates_ul_min=[
                        pump0_withdraw_rate_ul_min,
                        pump1_withdraw_rate_ul_min,
                    ],
                    withdraw_volumes_ul=[
                        pump0_withdraw_volume_ul,
                        pump1_withdraw_volume_ul,
                    ],
                )
                ...
                local.lib.dispensing.methods.set_valves_and_infuse(
                    ...
                    infuse_rates_ul_min=[
                        min(
                            self._devices.PUMP.get_max_rate_ul_min(self.pump0_input),
                            50000,
                        ),
                        min(
                            self._devices.PUMP.get_max_rate_ul_min(self.pump1_input),
                            50000,
                        ),
                    ],
                    infuse_volumes_ul=[
