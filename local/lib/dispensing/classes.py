...
class Devices(object):
    ...
    """
    PUMP: aqueduct.devices.trcx.obj.TRCX = None
    SCALE: aqueduct.devices.ohsa.obj.OHSA = None
...
class Data(object):
    ...
    def update_data(self) -> None:
        ...
        :return:
        """
        if INTERNAL and isinstance(self._devices.SCALE, aqueduct.devices.ohsa.obj.OHSA):
            self.W1 = self._devices.SCALE.weight(SCALE_INDEX)
...
class ReactionProcessHandler(object):
    ...
    def do_process(self):
        ...
        # the balance is only used for internal testing, not present for final instrument
        if INTERNAL:
            self._devices.SCALE.start()
