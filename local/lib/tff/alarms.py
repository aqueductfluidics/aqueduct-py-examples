...
class Alarm(object):
    ...
    def get_target_mass(self) -> Union[float, int, None]:
        ...
        :return:
        """
        if self._process.current_phase == local.lib.tff.classes.Process.INITIAL_CONC_PHASE:
            return self._process.init_conc_target_mass_g
        elif self._process.current_phase == local.lib.tff.classes.Process.DIAFILT_PHASE:
            return self._process.diafilt_target_mass_g
        elif self._process.current_phase == local.lib.tff.classes.Process.FINAL_CONC_PHASE:
...
class VolumeAccumulationAlarm(Alarm):
    ...
    def handle(self):
        ...
        if isinstance(rates, local.lib.tff.data.TrailingRates):
            rates.print()
