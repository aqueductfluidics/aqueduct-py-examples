...
import local.lib.lnp.definitions
...
def pump_ramp(
        ...
        watchdog: "local.lib.lnp.classes.Watchdog" = None
) -> int:
    """
    ...
    :rtype pump: local.lib.lnp.classes.aqueduct.devices.mfpp.obj.MFPP
    ...
    :param scale3_target_mass_g: target mass for SCALE3 in grams, if reading exceeds this value will break from the
        the loop early
    :param devices_obj:
    :rtype devices_obj: local.lib.lnp.classes:Devices
    :param data:
    :rtype data: local.lib.lnp.classes:Data
    :param watchdog:
    :rtype watchdog: local.lib.lnp.classes.Watchdog
    ...
    while target_pump1_ml_min < end_flowrate_ml_min:
        ...
        if time.time() > timeout:
            ...
            return local.lib.lnp.definitions.STATUS_TIMED_OUT
        ...
        while time.time() < time_loop_start + rate_change_interval_s:
            ...
            if scale3_target_mass_g is not None:
                if data.W3 is not None and data.W3 >= scale3_target_mass_g:
                    ...
                    return local.lib.lnp.definitions.STATUS_TARGET_MASS_HIT
        ...
        print("[RAMP] Adjusting {} rate to {:.2f} mL/min".format(pump_name,
              target_pump1_ml_min))
        pump.change_speed(target_pump1_ml_min)

    print("[RAMP] Completed {} ramp up.".format(pump_name))

    return local.lib.lnp.definitions.STATUS_OK
...
def pumps_2_and_3_ramp(
    ...
    if isinstance(devices_obj.PUMP2, aqueduct.devices.mfpp.obj.MFPP):
        print(
            "[DUAL RAMP] Starting PUMP2 at {:.2f} mL/min".format(pump2_start_flowrate_ml_min))
    ...
    print(
        "[DUAL RAMP] Starting PUMP3 at {:.2f} mL/min".format(pump3_start_flowrate_ml_min))
    ...
    pump2_rate_ml_min_range: List[float] = local.lib.lnp.helpers.get_flowrate_range(
        start_flow_rate=pump2_start_flowrate_ml_min,
        end_flow_rate=pump2_end_flowrate_ml_min,
        steps=number_rate_changes)

    pump3_rate_ml_min_range: List[float] = local.lib.lnp.helpers.get_flowrate_range(
    ...
    for pump2_rate_ml_min, pump3_rate_ml_min in zip(pump2_rate_ml_min_range[1::], pump3_rate_ml_min_range[1::]):
        ...
        print(
            "[DUAL RAMP] Adjusting PUMP3 rate to {:.2f} mL/min".format(pump3_rate_ml_min))
        devices_obj.PUMP3.change_speed(pump3_rate_ml_min)

    # check all alarms
    if isinstance(watchdog, local.lib.lnp.classes.Watchdog):
...
def open_pinch_valve(
    ...
    while data.PV < target_pct_open:
        ...
        print("[OPEN] Adjusting pinch valve to {} open, final target {}".format(
            local.lib.lnp.helpers.format_float(increment_target_pct_open, 4),
            local.lib.lnp.helpers.format_float(target_pct_open, 4)
        ))
...
def monitor(
        ...
        data: "local.lib.lnp.classes.Data" = None,
        watchdog: "local.lib.lnp.classes.Watchdog" = None,
        process: "local.lib.lnp.classes.Process" = None,
) -> None:
    """
    ...
    :param local.lib.lnp.classes.Devices devices_obj:
    :param local.lib.lnp.classes.Data data:
    :param local.lib.lnp.classes.Watchdog watchdog:
    :param local.lib.lnp.classes.Process process:
    :return: None
    """
    ...
    while time_tried_s < timeout_min * 60:
        ...
        try:
            ...
            while data.P3 > target_p3_psi + WINDOW_PSI and time_tried_s < timeout_min * 60:
                ...
                target_pct_open: float = data.PV + adj
                devices_obj.PV.set_position(target_pct_open, record=True)
                print("[LOCK IN] adjusting pinch valve to {} open".format(local.lib.lnp.helpers.format_float(
                    target_pct_open, 4)))
                # delay to allow response in pressure
                time.sleep(VALVE_DELAY_S)
                time_tried_s += VALVE_DELAY_S
